...
if (!registeredExtension) {
  console.error('Extension failed to register — Scratch.extensions.register was never called.');
  process.exit(1);
}
...
// Numeric values stored as actual numbers (not strings)
...
// Regression: container nodes (objects/arrays) must never be matched directly —
// only leaf primitives are compared; searching for a container's string coercion
// should return '' (not found).
ext.dict_manage({ DICT: 'test_nested_obj', ACTION: 'load JSON', DATA: '{"inner":{"x":1}}' });
assert('container regression: object node not matched as target', ext.dict_find_path({ VAL: '{"x":1}', DICT: 'test_nested_obj' }), '');

ext.array_init({ DICT: 'test_nested_arr' });
ext.array_push({ VAL: [1, 2], KEY: '', DICT: 'test_nested_arr' });
assert('container regression: array node not matched as target', ext.dict_find_path({ VAL: '1,2', DICT: 'test_nested_arr' }), '');
